...
const HbsOptions = {
  ignorePartials: true,
  batch: [`${options.paths.src.base}/template`],
};
...
//Development Tasks
function devHTML() {
  return src(`${options.paths.src.base}/**/*.hbs`)
    .pipe(handlebars(data, HbsOptions))
...
function devFonts() {
  return src(`${options.paths.src.base}/fonts/**/*`).pipe(
    dest(options.paths.dist.base + "/fonts")
  );
}
...
function watchFiles() {
  watch(
    [
      ...
      `${options.paths.src.base}/template/*.hbs`,
    ],
